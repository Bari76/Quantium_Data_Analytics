...
code!
Page 120200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
...
#Page 220200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
#data[, YEARMONTH := "%Y%m" ]
...
#Page 320200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
```
...
Page 420200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
...
trial_store <- 77
corr_nSales <- calculateCorrelation(preTrialMeasures, quote(totSales),trial_store)[order(-corr_measure)]
corr_nSales
corr_nCustomers <- calculateCorrelation(preTrialMeasures, quote(nCustomers),trial_store )[order(-corr_measure)]
...
#mag_measure
corr_weight <- 0.5
score_nSales <- merge(corr_nSales,magnitude_nSales , by =c("Store1","Store2") )[, scoreNSales := (corr_measure + mag_measure)/2 ][order(-scoreNSales)]
score_nSales
score_nCustomers <- merge(corr_nCustomers,magnitude_nCustomers , by =c("Store1","Store2") )[, scoreNCust := (corr_measure+mag_measure)/2][order(-scoreNCust)]
score_nCustomers
```
...
pastCustomers <- measureOverTimeCusts[, numberCustomers := mean(nCustomers),
                                      by = c("YEARMONTH", "Store_type")][YEARMONTH < 201906]
...
```
## Assessment of trial
Page 620200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
...
percentageDiff <- merge(scaledControlSales[, c("YEARMONTH", "controlSales")],
                        ...
                        by = "YEARMONTH")[, percentageDiff := abs(controlSales - totSales) /
                                          controlSales]
glimpse(percentageDiff)
```
Let's see if the difference is significant!
```{r}
#### As our null hypothesis is that the trial period is the same as the pre-trial
#period, lets take the standard deviation based on the scaled percentage difference
...
#Page 720200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
...
geom_line() +
labs(x = "Month of operation", y = "Total sales", title = "Total sales by month")
#Page 820200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
```
...
Page 920200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
interval"]
#### Control store 5th percentile
...
#Page 1020200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
corr_weight <- 0.5
...
][order(-finalControlStore)][2, Store2]
control_store
```
...
labs(x = "Month of operation", y = "Total number of customers",
     title = "Total number of customers by month")
```
Page 1120200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
...
#Page 1320200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
...
```
Page 1420200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
...
Again, let's check visually if the drivers are indeed similar in the period before
the trial.
Page 1520200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
We'll look at total sales first.
```{r}
#### Visual checks on trends based on the drivers
...
sscalingFactorForControlSales <- preTrialMeasures[Store_nbr == trial_store & YEARMONTH < 201902,
                                                 sum(totSales)] /
...
#in the pre-trial period
stdDev <- sd(percentageDiff[YEARMONTH < 201902, percentageDiff])
#Page 1620200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
degreesOfFreedom <- 7
#### Trial and control store total sales
measureOverTimeSales <- measureOverTime
pastSales <- measureOverTimeSales[, Store_type := ifelse(Store_nbr == trial_store, "Trial",
...
#period, lets take the standard deviation based on the scaled percentage difference
#in the pre-trial period
#Page 1720200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd
stdDev <- sd(percentageDiff[YEARMONTH < 201902, percentageDiff])
